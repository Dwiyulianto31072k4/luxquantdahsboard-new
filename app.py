...
def get_secret(key, default=None):
    """Get secret from ENV first, then st.secrets only if we're in local development."""
    # Always check environment variables first
    v = os.getenv(key)
    if v is not None:
        return v
    
    # Only check st.secrets if we're likely in local development
    # (detect by checking if we're not in a typical production environment)
...
class DataManager:
    ...
    def _get_credentials(self) -> Dict[str, Any]:
        """Get Google Sheets credentials – prefer secret files, then ENV (no st.secrets for production)."""
